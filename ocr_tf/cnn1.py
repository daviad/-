...
W_fc1 = weight_variable([16 * 20 * 32, 512])
b_fc1 = bias_variable([512])
h_pool2_flat = tf.reshape(h_pool2, [-1, 16 * 20 * 32])
h_fc1 = tf.nn.elu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)
keep_prob = tf.placeholder(tf.float32)
h_fc1_drop = tf.nn.dropout(h_fc1, keep_prob)

W_fc2 = weight_variable([512, train_data.category])
...
for i in range(1010):
    batch = train_data.batch_next(60)
    if i % 80 == 0:
