...
def evaluation(img_path, ckpt_path):  
  ...
  if len(face) > 0:  
    for rect in face:  
      ...
      face_detect_img_path = '../eval_images/' + random_str + '.jpg'  
      ...
      cv2.imwrite('../eval_images/' + random_str + '.jpg', img[y:y+h, x:x+w])  
  
      target_image_path = '../eval_images/' + random_str + '.jpg'  
  else:  
    print('image:No Face')  
    return  
  f.close()  
...
if __name__ == '__main__':  
  ...
  for image_path in TEST_IMAGE_PATHS:  
    evaluation('../eval_images/'+image_path, '../ckpt/model.ckpt')  